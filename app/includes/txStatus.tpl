<main class="tab-pane block--container active" ng-if="globalService.currentTab==globalService.tabs.txStatus.id" ng-controller='txStatusCtrl' role="main" ng-cloak>
  ...
  <section class="block txstatus__2" ng-show="txInfo.status==txStatus.mined || txInfo.status==txStatus.notFound || txInfo.status==txStatus.found">
    ...
    <div class="cont-md" ng-show="txInfo.status == txStatus.mined">
      ...
      <h5> <a href="https://explorer.whalecoin.org/#/transaction/{{ tx.hash }}" target="_blank" rel="noopener"> {{ tx.hash }} </a> </h5>
    ...
    <table class="table table-striped txstatus__2 cont-md"> <!-- ng-show="tx.status=='foundOnChain' || foundInPending'"-->
      <tbody>
        <tr>
          ...
          <td><a href="https://explorer.whalecoin.org/#/transaction/{{ txInfo.hash }}" target="_blank" rel="noopener"> {{ txInfo.hash }} </a></td>
        </tr>
        <tr>
          <td translate="OFFLINE_Step1_Label_1">From Address</td>
          <td><a href="https://explorer.whalecoin.org/#/address/{{ txInfo.from }}" target="_blank" rel="noopener"> {{ txInfo.from }} </a></td>
        </tr>
        <tr>
          <td translate="OFFLINE_Step2_Label_1">To Address</td>
          <td><a href="https://explorer.whalecoin.org/#/address/{{ txInfo.to }}" target="_blank" rel="noopener"> {{ txInfo.to }} </a></td>
        </tr>
        <tr>
        ...
        <tr>
          <td>
            <a class="account-help-icon" href="https://mywhalewallet.groovehq.com/knowledge_base/topics/what-is-nonce" target="_blank" rel="noopener">
        ...
        <tr>
          <td>
            <a class="account-help-icon" href="https://mywhalewallet.groovehq.com/knowledge_base/topics/what-is-gas" target="_blank" rel="noopener">
              <img src="images/icon-help.svg" class="help-icon" />
